...
import { AgreementCalculationSettingsIndicator } from "./AgreementCalculationSettingsIndicator";
import { shouldShowAgreementCalculationIndicator } from "../../../utils/agreementSelected";
...
function DataViewInner({
  ...
  const columnHeaderExtra = useCallback(
    ({ parent, original, help }, decoration) => {
      ...
      if (
        isAgreementColumn &&
        shouldShowAgreementCalculationIndicator(agreementFeatureFlagsActive, view?.agreement_selected)
      ) {
        children.push(
          <AgreementCalculationSettingsIndicator
            key="agreement-settings-indicator"
            agreementFilters={view?.agreement_selected}
          />,
        );
      }
      ...
      // A column is a clickable button when: agreement with both FFs ON, or agreement_selected (always a button)
      const isInteractiveAgreementColumn =
        (isAgreementColumn && agreementFeatureFlagsActive) ||
    ...
    [agreementFeatureFlagsActive, agreementSelectedKey, view],
  );
...
DataViewInner.displayName = "DataView";

export const DataView = injector(observer(DataViewInner));
